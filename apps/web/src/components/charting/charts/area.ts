...
export class AreaRenderer extends ChartRendererBase {
  buildOptions (): EChartsOption {
    ...
    if (dimensions.length === 1) {
      metrics.forEach((metric, mIdx) => {
        ...
        series.push({
          ...
          smooth: true,
          areaStyle: { opacity: 0.8, color: theme[mIdx % theme.length] },
      ...
      seriesGroups.forEach((group, gIdx) => {
        metrics.forEach((metric, mIdx) => {
          ...
          series.push({
            ...
            smooth: true,
            areaStyle: { opacity: 0.8, color: theme[colorIdx] },
            label: { show: this.config.settings?.showDataLabels !== false },
    ...
    return {
      ...
      xAxis: { type: 'category', data: xValues.filter((v): v is string | number => v !== null && v !== undefined && (typeof v === 'string' || typeof v === 'number')), boundaryGap: false, name: this.getDimLabel(xDim) },
